...
public class GridFlowItemMetadataResolver : DungeonEventListener
{
    ...
    {
        var items = GetDungeonOwnedComponents<GridFlowItemMetadataComponent>(dungeon);
        var itemMap = new Dictionary<string, GridFlowItemMetadataComponent>();
        ...
        {
            var oldKeys = GetDungeonOwnedComponents<GridFlowDoorKeyComponent>(dungeon);
            foreach (var key in oldKeys)
            {
                key.lockRefs = new GridFlowDoorLockComponent[0];
            }
            var oldLocks = GetDungeonOwnedComponents<GridFlowDoorLockComponent>(dungeon);
            foreach (var lockComponent in oldLocks)
            {
                lockComponent.validKeyRefs = new GridFlowDoorKeyComponent[0];
        ...
        foreach (var item in items)
        {
            if (item.itemType == GridFlowGraphItemType.Key)
            {
                var keyComponent = FindOrAddComponent<GridFlowDoorKeyComponent>(item.gameObject);
                keyComponent.keyId = item.itemId;

                var lockComponents = new List<GridFlowDoorLockComponent>();
                for (int i = 0; i < item.referencedItemIds.Length; i++) 
                {
                    ...
                    if (refItem.itemType == GridFlowGraphItemType.Lock)
                    {
                        var lockComponent = FindOrAddComponent<GridFlowDoorLockComponent>(refItem.gameObject);
                        lockComponents.Add(lockComponent);

                        var keyRefs = new List<GridFlowDoorKeyComponent>(lockComponent.validKeyRefs);
            ...
            else if (item.itemType == GridFlowGraphItemType.Lock)
            {
                var lockComponent = FindOrAddComponent<GridFlowDoorLockComponent>(item.gameObject);
