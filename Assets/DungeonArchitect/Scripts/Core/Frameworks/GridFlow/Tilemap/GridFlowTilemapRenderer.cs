...
namespace DungeonArchitect.Builders.GridFlow.Tilemap
{
    ...
    public class GridFlowTilemapRenderer
    {
        ...
        static Color GetEdgeColor(GridFlowTilemapEdgeType edgeType)
        {
            if (edgeType == GridFlowTilemapEdgeType.Wall) return Color.red;
            else if (edgeType == GridFlowTilemapEdgeType.Fence) return Color.black;
            else if (edgeType == GridFlowTilemapEdgeType.Door) return Color.blue;
            ...
            for (int y = 0; y <= tilemap.Height; y++)
            {
                for (int x = 0; x <= tilemap.Width; x++)
                {
                    ...
                    if (edgeH.EdgeType != GridFlowTilemapEdgeType.Empty)
                    {
                        ...
                        if (edgeH.EdgeType == GridFlowTilemapEdgeType.Door)
                        {
                            offset *= 3;
                        }
                    ...
                    if (edgeV.EdgeType != GridFlowTilemapEdgeType.Empty)
                    {
                        ...
                        if (edgeV.EdgeType == GridFlowTilemapEdgeType.Door)
                        {
                            offset *= 3;
                        }
        ...
        static void DrawTileEdge(float x0, float y0, float x1, float y1, GridFlowTilemapEdge edge, GridFlowTilemapRenderResources resources, Color color, Material material, Dictionary<Material, List<TilemapRenderQuad>> quadsByMaterial)
        {
            var v0 = new TilemapRenderVert(new Vector2(x0, y0), new Vector2(0, 1));
            var v1 = new TilemapRenderVert(new Vector2(x0, y1), new Vector2(0, 0));
            var v2 = new TilemapRenderVert(new Vector2(x1, y1), new Vector2(1, 0));
            ...
            if (edge != null && edge.EdgeType == GridFlowTilemapEdgeType.Door)
            {
                var doorMeta = edge.Userdata as GridFlowTilemapCellDoorInfo;
                if (doorMeta != null)
                {
                    if (doorMeta.oneWay)
                    {
