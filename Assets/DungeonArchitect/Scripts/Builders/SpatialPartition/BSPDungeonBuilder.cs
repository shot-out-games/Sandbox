        ...
        void EmitLevelMarkers()
        {
                ...
                if (node.children.Length == 0)
                {
                    ...
                    for (int x = 0; x < paddedBounds.Size.x; x++)
                    {
                        for (int z = 0; z < paddedBounds.Size.z; z++)
                        {
                            ...
                            EmitMarker(BSPDungeonConstants.GroundRoom, transform, new IntVector(x, 0, z), -1);
                        }
                    }
                }
            }
            ...
            foreach (var connection in bspModel.connections)
            {
                ...
                // Emit the doors
                {
                    ...
                    EmitMarker(BSPDungeonConstants.Door, transform, pos0, -1);

                    transform = Matrix4x4.TRS(worldPos1, Quaternion.Euler(0, angle1, 0), Vector3.one);
                    EmitMarker(BSPDungeonConstants.Door, transform, pos1, -1);
                ...
                for (int x = x0; x <= x1; x++)
                {
                    for (int z = z0; z <= z1; z++)
                    {
                        ...
                        EmitMarker(BSPDungeonConstants.GroundCorridor, transform, new IntVector(x, 0, z), -1);
                    }
                }
                ...
                if (x0 == x1)
                {
                    for (int z = z0; z <= z1; z++)
                    {
                        ...
                        EmitMarker(BSPDungeonConstants.WallCorridor, transform, new IntVector(x0, 0, z), -1);
                        ...
                        EmitMarker(BSPDungeonConstants.WallCorridor, transform, new IntVector(x1, 0, z), -1);
                    }
                }
                else
                {
                    for (int x = x0; x <= x1; x++)
                    {
                        ...
                        EmitMarker(BSPDungeonConstants.WallCorridor, transform, new IntVector(x, 0, z0), -1);
                        ...
                        EmitMarker(BSPDungeonConstants.WallCorridor, transform, new IntVector(x, 0, z1), -1);
                    }
                }
            }
            ...
            foreach (var node in bspModel.nodes)
            {
                ...
                // Emit he walls 
                {
                    for (int x = x0; x < x1; x++)
                    {
                        if (!doorPositions.Contains(new IntVector(x, 0, z0)))
                        {
                            var worldPos = Vector3.Scale(new Vector3(x + 0.5f, 0, z0), gridSize3D);
                            var transform = Matrix4x4.TRS(worldPos, Quaternion.Euler(0, 0, 0), Vector3.one);
                            EmitMarker(BSPDungeonConstants.WallRoom, transform, new IntVector(x, 0, z0), -1);
                        }
                        ...
                        if (!doorPositions.Contains(new IntVector(x, 0, z1)))
                        {
                            ...
                            EmitMarker(BSPDungeonConstants.WallRoom, transform, new IntVector(x, 0, z1), -1);
                        }
                    }
                    ...
                    for (int z = z0; z < z1; z++)
                    {
                        ...
                        if (!doorPositions.Contains(new IntVector(x0, 0, z)))
                        {
                            var worldPos = Vector3.Scale(new Vector3(x0, 0, z + 0.5f), gridSize3D);
                            var transform = Matrix4x4.TRS(worldPos, Quaternion.Euler(0, 90, 0), Vector3.one);
                            EmitMarker(BSPDungeonConstants.WallRoom, transform, new IntVector(x0, 0, z), -1);
                        }
                        ...
                        if (!doorPositions.Contains(new IntVector(x1, 0, z)))
                        {
                            ...
                            EmitMarker(BSPDungeonConstants.WallRoom, transform, new IntVector(x1, 0, z), -1);
                        }
                    }
                }
                ...
                // Emit the wall separators
                {
                    for (int x = x0; x <= x1; x++)
                    {
                        ...
                        var transform = Matrix4x4.TRS(worldPos, Quaternion.Euler(0, 0, 0), Vector3.one);
                        EmitMarker(BSPDungeonConstants.WallSeparator, transform, new IntVector(x, 0, z0), -1);

                        worldPos = Vector3.Scale(new Vector3(x, 0, z1), gridSize3D);
                        transform = Matrix4x4.TRS(worldPos, Quaternion.Euler(0, 0, 0), Vector3.one);
                        EmitMarker(BSPDungeonConstants.WallSeparator, transform, new IntVector(x, 0, z1), -1);
                    }
                    ...
                    for (int z = z0 + 1; z <= z1 - 1; z++)
                    {
                        ...
                        var transform = Matrix4x4.TRS(worldPos, Quaternion.Euler(0, 0, 0), Vector3.one);
                        EmitMarker(BSPDungeonConstants.WallSeparator, transform, new IntVector(x0, 0, z), -1);

                        worldPos = Vector3.Scale(new Vector3(x1, 0, z), gridSize3D);
                        transform = Matrix4x4.TRS(worldPos, Quaternion.Euler(0, 0, 0), Vector3.one);
                        EmitMarker(BSPDungeonConstants.WallSeparator, transform, new IntVector(x1, 0, z), -1);
                    }
                }
            }
