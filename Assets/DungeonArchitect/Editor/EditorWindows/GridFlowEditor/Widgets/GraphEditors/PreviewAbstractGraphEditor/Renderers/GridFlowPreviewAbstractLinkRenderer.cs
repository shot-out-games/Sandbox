...
using UnityEngine;
using UnityEditor;
using DungeonArchitect.Builders.GridFlow.Graphs.Preview;
...
{
    public class GridFlowPreviewAbstractLinkRenderer : IGraphLinkRenderer
    {
        ...
        public void DrawGraphLink(UIRenderer renderer, GraphRendererContext rendererContext, GraphLink link, GraphCamera camera)
        {
            ...
            if (previewLink != null)
            {
                bool directional = previewLink.AbstractLinkState.Directional;

                var color = directional ? lineColorDirected : lineColorUndirected;
                if (previewLink.AbstractLinkState.OneWay)
                {
                ...
                if (directional)
                {
                    ...
                    {
                        float itemPadding = 2;
                        var items = previewLink.AbstractLinkState.Items;
